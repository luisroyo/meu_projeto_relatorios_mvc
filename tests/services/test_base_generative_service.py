...
def test_base_service_init_no_api_key(caplog):
    ...
    with patch('os.getenv') as mock_getenv:
        def getenv_side_effect(key, default=None):
            if key == 'GOOGLE_API_KEY':
                return None
            # Não precisamos mais mockar LOG_LEVEL aqui se o BaseGenerativeService
            # não usa os.getenv('LOG_LEVEL') para configurar seu logger, ou se
            # o default já é suficiente para o logger BaseGenerativeService emitir CRITICAL.
            return os.environ.get(key, default)
        mock_getenv.side_effect = getenv_side_effect

        # Captura CRITICAL e mensagens de nível superior.
        ...
        # Verifica APENAS a mensagem CRITICAL que é logada antes de levantar RuntimeError.
        # O BaseGenerativeService, quando a chave não é encontrada no _check_api_key,
        # retorna False, e o __init__ então loga a mensagem CRITICAL e levanta a exceção.
        # A mensagem de erro exata que vai para o logger.critical é o error_message.
        ...
        # A linha abaixo que causava a falha foi REMOVIDA, pois com logging.CRITICAL,
        # a mensagem de ERROR não é capturada.
        # # assert "API Key do Google (GOOGLE_API_KEY) não encontrada nas variáveis de ambiente." in caplog.text
