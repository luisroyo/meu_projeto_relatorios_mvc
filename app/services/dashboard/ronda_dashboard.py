...
from app import db
from app.models import User, Ronda, Condominio, EscalaMensal
from app.utils.date_utils import parse_date_range
from .helpers import date_utils, chart_data
...
def get_ronda_dashboard_data(filters):
    ...
    rondas_por_condominio_q = filters_helper.apply_ronda_filters(rondas_por_condominio_q, filters, date_start_range, date_end_range)
    ...
    duracao_somas_q = filters_helper.apply_ronda_filters(duracao_somas_q, filters, date_start_range, date_end_range)
    duracao_somas_raw = duracao_somas_q.group_by(Condominio.nome).all()
    
    # [ALTERADO] Lógica de cálculo movida para o helper de KPIs
    dados_ordenados = kpis_helper.calculate_average_duration_by_condominio(duracao_somas_raw)
    ...
    rondas_por_turno_q = filters_helper.apply_ronda_filters(rondas_por_turno_q, filters, date_start_range, date_end_range)
    ...
    rondas_por_supervisor_q = filters_helper.apply_ronda_filters(rondas_por_supervisor_q, filters, date_start_range, date_end_range)
    ...
    rondas_por_dia_q = filters_helper.apply_ronda_filters(rondas_por_dia_q, filters, date_start_range, date_end_range)
    ...
    if (date_end_range - date_start_range).days < 366:
        ronda_date_labels = date_utils.generate_date_labels(date_start_range, date_end_range)
        ronda_activity_data = chart_data.fill_series_with_zeros(rondas_por_dia, ronda_date_labels)
    ...
    base_kpi_query = filters_helper.apply_ronda_filters(db.session.query(Ronda), filters, date_start_range, date_end_range)
    
    # [ALTERADO] Lógica de cálculo movida para o helper de KPIs
    total_rondas, duracao_media_geral, supervisor_mais_ativo = kpis_helper.calculate_main_ronda_kpis(base_kpi_query, supervisor_labels)
    media_rondas_dia = kpis_helper.calculate_average_rondas_per_day(total_rondas, filters, date_start_range, date_end_range)
