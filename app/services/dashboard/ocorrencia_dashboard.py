...
from app.models import (
    User, Condominio, Ocorrencia, OcorrenciaTipo, Colaborador
)
...
def get_ocorrencia_dashboard_data(filters):
    ...
    base_kpi_query = ocorrencia_service.apply_ocorrencia_filters(base_kpi_query, filters)
    ...
    ocorrencias_abertas = base_kpi_query.filter(Ocorrencia.status.in_(status_abertos)).count()
    ...
    ocorrencias_por_tipo_q = ocorrencia_service.apply_ocorrencia_filters(ocorrencias_por_tipo_q, filters)
    ...
    ocorrencias_por_condominio_q = ocorrencia_service.apply_ocorrencia_filters(ocorrencias_por_condominio_q, filters)
    ...
    ocorrencias_por_dia_q = ocorrencia_service.apply_ocorrencia_filters(ocorrencias_por_dia_q, filters)
    ...
    date_start_range, date_end_range = parse_date_range(filters.get('data_inicio_str'), filters.get('data_fim_str'))
    if (date_end_range - date_start_range).days < 366:
        evolucao_date_labels = date_utils.generate_date_labels(date_start_range, date_end_range)
        evolucao_ocorrencia_data = chart_data.fill_series_with_zeros(ocorrencias_por_dia, evolucao_date_labels)
    ...
    ultimas_ocorrencias_q = ocorrencia_service.apply_ocorrencia_filters(ultimas_ocorrencias_q, filters)
    ...
    top_colaboradores_q = ocorrencia_service.apply_ocorrencia_filters(top_colaboradores_q, filters)
