...
def _get_monthly_aggregation(model, date_column, year):
    ...
    return (
        db.session.query(func.to_char(date_column, "YYYY-MM"), func.count(model.id))
        .filter(func.to_char(date_column, "YYYY") == str(year))
        .group_by(func.to_char(date_column, "YYYY-MM"))
...
def get_monthly_comparison_data(year=None):
    ...
    ocorrencias_raw = _get_monthly_aggregation(
        Ocorrencia, Ocorrencia.data_hora_ocorrencia, year
    )
