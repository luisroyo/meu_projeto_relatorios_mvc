...
class JustificativaTrocaPlantaoService(BaseGenerativeService):
    def __init__(
        ...
        template_filename="justificativa_troca_plantao_template.txt",
    ):
        ...
        try:
            ...
            self.logger.info(
                f"Template '{template_filename}' carregado para JustificativaTrocaPlantaoService."
            )
        except Exception as e:
            self.logger.error(
                f"Erro ao carregar template '{template_filename}' para JustificativaTrocaPlantaoService: {e}",
                exc_info=True,
            )
            raise RuntimeError(
                f"Falha ao carregar template para JustificativaTrocaPlantaoService: {e}"
            ) from e

    def _construir_prompt(self, dados_justificativa: dict) -> str:
        if not self._template:
            raise RuntimeError(
                "Template de justificativa de troca de plantão não está carregado."
            )
        if not isinstance(dados_justificativa, dict):
            raise ValueError(
                "Os dados para a justificativa de troca de plantão devem ser um dicionário."
            )
        try:
            # O template 'justificativa_troca_plantao_template.txt' espera placeholders como {{ colaborador_a_nome }}
            prompt_final = self._template.render(
                dados_justificativa
            )  # Passa o dicionário diretamente
            self.logger.debug(
                f"Prompt para JustificativaTrocaPlantao (100 chars): {prompt_final[:100]}..."
            )
            return prompt_final
        except Exception as e:
            self.logger.error(
                f"Erro ao renderizar template de justificativa de troca com Jinja2: {e}",
                exc_info=True,
            )
            raise ValueError(
                f"Erro ao construir o prompt para a justificativa de troca: {str(e)}"
            ) from e

    def gerar_justificativa_troca(self, dados_justificativa: dict) -> str:
        self.logger.info(
            f"Gerando justificativa de troca de plantão com dados: {dados_justificativa}"
        )
        ...
        try:
            ...
            self.logger.exception(
                "Erro inesperado ao gerar justificativa de troca de plantão:"
            )
            raise RuntimeError(
                f"Erro inesperado no serviço de justificativa de troca: {str(e)}"
            ) from e
