...
class PatrimonialReportService(BaseGenerativeService):
    def __init__(self, model_name="gemini-1.5-flash-latest", template_filename="patrimonial_security_report_template.txt"):
        super().__init__(model_name=model_name) # Chama o construtor da BaseGenerativeService
        ...
        try:
            ...
            self.logger.error(f"Erro ao carregar template '{template_filename}' para PatrimonialReportService: {e}", exc_info=True)
            # Decide como tratar o erro. Relançar é uma boa opção para indicar que o serviço não está funcional.
            raise RuntimeError(f"Falha ao carregar template para PatrimonialReportService: {e}") from e

    def _construir_prompt_relatorio_patrimonial(self, dados_brutos: str) -> str:
        if not self._template:
            self.logger.error("Tentativa de construir prompt, mas o template não foi carregado.")
        ...
        try:
            ...
            raise ValueError(f"Erro ao construir o prompt para o relatório patrimonial: {str(e)}") from e
    ...
    def gerar_relatorio_seguranca(self, dados_brutos_relatorio: str) -> str:
        ...
        try:
            prompt_para_ia = self._construir_prompt_relatorio_patrimonial(dados_brutos_relatorio)
            ...
            self.logger.info("Relatório de segurança patrimonial gerado pela IA com sucesso.")
            return texto_processado
        except ValueError as ve: # Captura erros de validação de dados ou problemas com o template
        ...
        except RuntimeError as rte: # Captura erros de configuração ou da chamada da IA na base
