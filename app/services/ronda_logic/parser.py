...
def _ajustar_data_evento_para_plantao(dt_obj, data_str_original, inicio_plantao, fim_plantao, log_entry_datetime=None):
    ...
    if inicio_plantao and fim_plantao:
        if not (inicio_plantao <= dt_obj < fim_plantao):
            ...
            if inicio_plantao <= dt_obj_dia_anterior < fim_plantao:
                logger.info(f"Heurística de intervalo: Ajustando data do evento para o dia anterior. Original: {dt_obj}, Ajustado: {dt_obj_dia_anterior}")
                return dt_obj_dia_anterior
            elif inicio_plantao <= dt_obj_dia_seguinte < fim_plantao:
                logger.info(f"Heurística de intervalo: Ajustando data do evento para o dia seguinte. Original: {dt_obj}, Ajustado: {dt_obj_dia_seguinte}")
                return dt_obj_dia_seguinte
    
    # Heurística 2: Baseada no timestamp do log (fallback)
    if log_entry_datetime and log_entry_datetime.time().hour < 7 and dt_obj.time().hour >= 18:
        if dt_obj.date() == log_entry_datetime.date():
            dt_obj_ajustado = dt_obj - timedelta(days=1)
            if not (inicio_plantao and fim_plantao) or (inicio_plantao <= dt_obj_ajustado < fim_plantao):
                logger.info(f"Heurística de timestamp: Ajustando data do evento para dia anterior. Original: {dt_obj}, Ajustado: {dt_obj_ajustado}")
                return dt_obj_ajustado

    return dt_obj # Retorna o objeto original se nenhum ajuste for necessário

def extrair_eventos_de_mensagem_simples(mensagem_raw: str, data_log_contexto: str, id_vtr_contexto: str, 
                                        linha_original_str: str, log_entry_datetime: datetime = None,
                                        inicio_plantao: datetime = None, fim_plantao: datetime = None):
    if not mensagem_raw or not data_log_contexto or data_log_contexto == config.FALLBACK_DATA_INDEFINIDA:
        return []
    ...
    for r_info in config.COMPILED_RONDA_EVENT_REGEXES:
        ...
        if match_evento:
            ...
            if hora_evento_raw is None: continue
            hora_formatada = normalizar_hora_capturada(hora_evento_raw)
            if not hora_formatada: continue
            
            try:
                dt_obj_inicial = datetime.strptime(f"{data_log_contexto} {hora_formatada}", "%d/%m/%Y %H:%M")
                dt_obj_final = _ajustar_data_evento_para_plantao(dt_obj_inicial, data_log_contexto, inicio_plantao, fim_plantao, log_entry_datetime)
                
                evento = {
                    "vtr": id_vtr_contexto or config.DEFAULT_VTR_ID, "tipo": r_info["tipo"],
                    "hora_str": hora_formatada, "data_str": dt_obj_final.strftime("%d/%m/%Y"),
                    "datetime_obj": dt_obj_final, "linha_original": linha_original_str
                }
                return [evento]
            except ValueError as ve:
                logger.error(f"Erro de data/hora ao parsear evento: {ve}. Linha: '{linha_original_str}'")
    return []

def extrair_eventos_de_bloco(bloco_linhas_orig: list, vtr_contexto_linha_prefixada: str, data_contexto_bloco: str, 
                             linha_original_referencia: str, log_entry_datetime_referencia: datetime = None,
                             inicio_plantao: datetime = None, fim_plantao: datetime = None):
    ...
    if bloco_linhas:
        ...
        match_vtr = config.REGEX_VTR_MENSAGEM_ALTERNATIVA.match(primeira_linha_bloco_limpa)
        if match_vtr:
    ...
    for idx, linha_b_raw in enumerate(bloco_linhas):
        linha_b_limpa = _limpar_e_normalizar_mensagem(linha_b_raw) if idx > 0 or not match_vtr else linha_b_raw
    ...
    for idx, linha_b_raw in enumerate(bloco_linhas):
        if idx in linhas_processadas_indices or not _limpar_e_normalizar_mensagem(linha_b_raw): continue
        if data_bloco_especifica and data_bloco_especifica != config.FALLBACK_DATA_INDEFINIDA:
            eventos_acumulados_bloco.extend(extrair_eventos_de_mensagem_simples(
                linha_b_raw, data_bloco_especifica, vtr_para_eventos_deste_bloco,
                f"{linha_original_referencia} (bloco_linha_raw: {linha_b_raw[:30]})",
                log_entry_datetime_referencia, inicio_plantao, fim_plantao
            ))
    
    for tipo_evento_bloco, hora_raw in [("inicio", hora_inicio_raw_bloco), ("termino", hora_termino_raw_bloco)]:
        if hora_raw:
            ...
            if hora_fmt and data_bloco_especifica and data_bloco_especifica != config.FALLBACK_DATA_INDEFINIDA:
                try:
                    ...
                    dt_obj_final = _ajustar_data_evento_para_plantao(dt_obj, data_bloco_especifica, inicio_plantao, fim_plantao, log_entry_datetime_referencia)
                    eventos_acumulados_bloco.append({
                        "vtr": vtr_para_eventos_deste_bloco or config.DEFAULT_VTR_ID, "tipo": tipo_evento_bloco, 
                        "hora_str": hora_fmt, "data_str": dt_obj_final.strftime("%d/%m/%Y"), 
                        "datetime_obj": dt_obj_final, "linha_original": f"{linha_original_referencia} (bloco_explicito)"
                    })
                except ValueError as ve: logger.error(f"Erro data/hora (bloco explícito): {ve}")
