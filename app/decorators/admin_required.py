    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated:
            current_app.logger.debug(
                f"Admin Required: Usuário não autenticado, redirecionando para login. Tentando acessar: {request.url}"
            )
            return redirect(url_for("auth.login", next=request.url))

        # DEBUG: Adicionado para verificar o estado do current_user
        print(
            f"--- DEBUG @admin_required: Usuário: {current_user.username}, Email: {current_user.email}, is_admin: {current_user.is_admin}, is_approved: {current_user.is_approved} ---"
        )
