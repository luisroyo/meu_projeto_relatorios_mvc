...
def listar_ocorrencias():
    ...
    return render_template('ocorrencia/list.html', title='Histórico de Ocorrências', ocorrencias_pagination=ocorrencias_pagination, condominios=condominios, supervisors=supervisors, status_list=status_list, selected_status=selected_status, selected_condominio_id=selected_condominio_id, selected_supervisor_id=selected_supervisor_id, selected_data_inicio=selected_data_inicio, selected_data_fim=selected_data_fim, filter_args=filter_args)
...
def add_orgao_publico():
    ...
    if not data or not data.get('nome'): return jsonify({'success': False, 'message': 'O nome do órgão é obrigatório.'}), 400
    nome_orgao = data['nome'].strip()
    if OrgaoPublico.query.filter_by(nome=nome_orgao).first(): return jsonify({'success': False, 'message': 'Este órgão público já existe.'}), 409
    try:
...
def add_colaborador():
    data = request.get_json()
    if not data or not data.get('nome_completo'): return jsonify({'success': False, 'message': 'O nome do colaborador é obrigatório.'}), 400
    nome_colaborador = data['nome_completo'].strip()
    if Colaborador.query.filter_by(nome_completo=nome_colaborador).first(): return jsonify({'success': False, 'message': 'Este colaborador já existe.'}), 409
    try:
        novo_colaborador = Colaborador(nome_completo=nome_colaborador, status='Ativo')
        db.session.add(novo_colaborador)
        db.session.commit()
        return jsonify({'success': True, 'id': novo_colaborador.id, 'nome_completo': novo_colaborador.nome_completo})
    except Exception as e:
        db.session.rollback()
