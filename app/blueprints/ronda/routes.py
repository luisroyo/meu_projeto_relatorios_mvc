...
def salvar_ronda():
    ...
    try:
        ronda_id = int(data.get('ronda_id')) if data.get('ronda_id') and str(data.get('ronda_id')).isdigit() and int(data.get('ronda_id')) > 0 else None
        ...
        if condominio_id_str == 'Outro':
            if not nome_condominio_outro: return jsonify({'success': False, 'message': 'O nome do condomínio é obrigatório.'}), 400
        ...
        if total == 0: return jsonify({'success': False, 'message': 'Não foi possível salvar: Nenhum evento de ronda válido foi encontrado no log fornecido.'}), 400

        # --- INÍCIO DA CORREÇÃO DE FUSO HORÁRIO PARA A RONDA ---
        ...
        if not supervisor_id_para_db:
            ...
            if escala: supervisor_id_para_db = escala.supervisor_id

        if ronda_id:
            ronda = db.get_or_404(Ronda, ronda_id)
            ronda.log_ronda_bruto, ronda.relatorio_processado, ronda.condominio_id = log_bruto, relatorio, condominio_obj.id
            ronda.data_plantao_ronda, ronda.escala_plantao, ronda.turno_ronda = data_plantao, escala_plantao, turno_ronda
            ronda.supervisor_id, ronda.total_rondas_no_log = supervisor_id_para_db, total
            ronda.primeiro_evento_log_dt, ronda.ultimo_evento_log_dt = primeiro_evento_utc, ultimo_evento_utc # Usa as variáveis UTC
            ronda.duracao_total_rondas_minutos = duracao
            mensagem_sucesso = 'Ronda atualizada com sucesso!'
        else:
            if Ronda.query.filter_by(condominio_id=condominio_obj.id, data_plantao_ronda=data_plantao, turno_ronda=turno_ronda).first():
                return jsonify({'success': False, 'message': 'Já existe uma ronda para este condomínio, data e turno.'}), 409
            ...
            ronda = Ronda(
                ...
                primeiro_evento_log_dt=primeiro_evento_utc, ultimo_evento_log_dt=ultimo_evento_utc, # Usa as variáveis UTC
                duracao_total_rondas_minutos=duracao, data_hora_inicio=datetime.now(timezone.utc)
            )
...
def listar_rondas():
    ...
    filter_params = {
        'condominio': request.args.get('condominio'), 'supervisor': request.args.get('supervisor', type=int),
    ...
    if active_filter_params.get('condominio'): query = query.join(Condominio).filter(Condominio.nome == active_filter_params['condominio'])
    if active_filter_params.get('supervisor'): query = query.filter(Ronda.supervisor_id == active_filter_params['supervisor'])
    if active_filter_params.get('turno'): query = query.filter(Ronda.turno_ronda == active_filter_params['turno'])
    ...
    if active_filter_params.get('data_inicio'):
        ...
        except (ValueError, TypeError): flash('Formato de data de início inválido.', 'warning')
    ...
    if active_filter_params.get('data_fim'):
        ...
        except (ValueError, TypeError): flash('Formato de data de fim inválido.', 'warning')
    ...
    if data_inicio_obj and data_fim_obj and (data_fim_obj - data_inicio_obj).days >= 0:
    ...
    return render_template(
        'ronda/list.html', title='Histórico de Rondas', rondas_pagination=rondas_pagination,
        filter_params=active_filter_params, condominios=Condominio.query.order_by(Condominio.nome).all(),
        ...
        total_rondas=total_rondas, duracao_media=duracao_media, media_rondas_dia=media_rondas_dia,
        supervisor_mais_ativo=supervisor_mais_ativo, **{f"selected_{k}": v for k, v in active_filter_params.items()}
    )

@ronda_bp.route('/detalhes/<int:ronda_id>')
@login_required
def detalhes_ronda(ronda_id):
    # CORREÇÃO: Removido o argumento 'Ronda' extra.
    ronda = Ronda.query.options(joinedload(Ronda.condominio), joinedload(Ronda.supervisor)).get_or_404(ronda_id)
