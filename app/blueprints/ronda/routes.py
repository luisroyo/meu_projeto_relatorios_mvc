...
def salvar_ronda():
    ...
    try:
        ...
        if not all([log_bruto, condominio_obj, data_plantao_str, escala_plantao]):
            return (
                jsonify({"success": False, "message": "Dados essenciais ausentes."}),
                400,
            )
        ...
        if total == 0:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Não foi possível salvar: Nenhum evento de ronda válido foi encontrado no log fornecido.",
                    }
                ),
                400,
            )
        ...
        if ronda_id:
            ...
            ronda.log_ronda_bruto, ronda.relatorio_processado, ronda.condominio_id = (
                log_bruto,
                relatorio,
                condominio_obj.id,
            )
            ronda.data_plantao_ronda, ronda.escala_plantao, ronda.turno_ronda = (
                data_plantao,
                escala_plantao,
                turno_ronda,
            )
            ronda.supervisor_id, ronda.total_rondas_no_log = (
                supervisor_id_para_db,
                total,
            )
            ronda.primeiro_evento_log_dt, ronda.ultimo_evento_log_dt = (
                primeiro_evento_utc,
                ultimo_evento_utc,
            )  # Usa as variáveis UTC
            ...
            if Ronda.query.filter_by(
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Já existe uma ronda para este condomínio, data e turno.",
                        }
                    ),
                    409,
                )
            ...
            ronda = Ronda(
                ...
                ultimo_evento_log_dt=ultimo_evento_utc,  # Usa as variáveis UTC
        ...
        return (
            jsonify(
                {"success": True, "message": mensagem_sucesso, "ronda_id": ronda.id}
            ),
            200,
        )
        ...
        return (
            jsonify(
                {"success": False, "message": f"Erro interno ao salvar ronda: {str(e)}"}
            ),
            500,
        )
...
def listar_rondas():
    ...
    active_filter_params = {
        k: v for k, v in filter_params.items() if v not in [None, ""]
    }
    ...
    if active_filter_params.get("condominio"):
        query = query.join(Condominio).filter(
            Condominio.nome == active_filter_params["condominio"]
        )
    ...
    total_rondas = (
        query.with_entities(func.sum(Ronda.total_rondas_no_log)).scalar() or 0
    )
    soma_duracao = (
        query.with_entities(func.sum(Ronda.duracao_total_rondas_minutos)).scalar() or 0
    )
    ...
    rondas_pagination = query.order_by(
        Ronda.data_plantao_ronda.desc(), Ronda.id.desc()
    ...
    return render_template(
        ...
        supervisors=User.query.filter_by(is_supervisor=True, is_approved=True)
        .order_by(User.username)
        .all(),
