...
def formatar_relatorio_rondas(
    ...
    if escala_plantao_input_str and isinstance(escala_plantao_input_str, str):
        ...
        if len(partes_escala) == 2:
            try:
                ...
                ):  # Se não vazio, usa o que foi digitado
    ...
    rondas_pareadas_ordenadas = sorted(
        [r for r in rondas_pareadas if r.get("inicio_dt")], key=lambda r: r["inicio_dt"]
    )
    # Adicionar rondas órfãs (apenas término) se necessário, talvez no final
    rondas_apenas_termino = [
        r for r in rondas_pareadas if not r.get("inicio_dt") and r.get("termino_dt")
    ]
    ...
    for ronda in rondas_pareadas_ordenadas:  # Processa primeiro as que têm início
        if ronda.get("inicio_dt") and isinstance(ronda["inicio_dt"], datetime):
            ...
            if ronda.get("termino_dt") and isinstance(ronda["termino_dt"], datetime):
                ...
                if (
                    ronda["termino_dt"] > ronda["inicio_dt"]
                ):  # Calcula duração se término for após início
    ...
    if rondas_apenas_termino:
        if not rondas_exibidas:
            linhas_saida.append("")  # Espaço se não houve rondas com início
    ...
    if (
        not rondas_exibidas and not rondas_apenas_termino
    ):  # Se nenhuma ronda (completa ou órfã) foi listada
        if alertas_pareamento:
            linhas_saida.append(
                "\u2003Nenhuma ronda para exibir (verifique observações/alertas)."
            )
        elif eventos_encontrados:  # Havia eventos, mas não formaram rondas
            linhas_saida.append(
                "\u2003Nenhuma ronda completa ou parcial para exibir (eventos detectados, mas não formaram rondas)."
            )
        else:  # Nenhum evento encontrado
            linhas_saida.append(
                "\u2003Nenhuma ronda para exibir (nenhum evento detectado)."
            )

    if (
        rondas_exibidas
        or rondas_apenas_termino
        or (eventos_encontrados and not alertas_pareamento and not rondas_pareadas)
    ):
        linhas_saida.append(
