...
def parear_eventos_parada(eventos: list):
    paradas_pareadas = []
    alertas_pareamento = []
    ...
    for evento in eventos:
        ...
        if evento["tipo"] == "inicio":
            ...
            if inicio_pendente_vtr_atual:
                alertas_pareamento.append(
                    f"⚠️ Início de parada para {inicio_pendente_vtr_atual['vtr']} às {inicio_pendente_vtr_atual['data_str']} {inicio_pendente_vtr_atual['hora_str']} "
                    f"sem término antes de novo início. (Linhas: '{inicio_pendente_vtr_atual['linha_original']}' e '{evento['linha_original']}')"
                )
                paradas_pareadas.append(
                    {
            ...
            if inicio_pendente_vtr_atual:
                ...
                alertas_pareamento.append(
                    f"⚠️ Término de parada para {evento['vtr']} às {evento['data_str']} {evento['hora_str']} "
    ...
    for vtr_id, inicio_pendente in inicios_pendentes_por_vtr.items():
        alertas_pareamento.append(
            f"⚠️ Início de parada para {inicio_pendente['vtr']} às {inicio_pendente['data_str']} {inicio_pendente['hora_str']} "
    ...
    return paradas_pareadas, alertas_pareamento, soma_duracoes_minutos
